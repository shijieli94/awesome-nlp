...
def filled_hist(ax, edges, values, bottoms=None, orientation="v", **kwargs):
    ...
    **kwargs
    ...
    if len(edges) - 1 != len(values):
        raise ValueError(f"Must provide one more bin edge than value not: " f"{len(edges)=} {len(values)=}")
...
def stack_hist(
    ax, stacked_data, sty_cycle, bottoms=None, hist_func=None, labels=None, plot_func=None, plot_kwargs=None
):
    """
    Parameters
    ----------
