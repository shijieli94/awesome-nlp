...
def convert_to_segment(args):
    ...
    def _segment_seqtag(src, tgt, num=None):
        ...
        for idx, (s, t) in enumerate(zip(src, tgt)):
            ...
            # count tokens
            if len(segs) > 0 and segs[-1][0] + max_toks < args.max_tokens and (num is None or segs[-1][1] < num):
                segs[-1][0] += max_toks
                segs[-1][1] += 1
    ...
    for corpus in corpuses:
        ...
        for idx, (src, tgt) in enumerate(zip(src_lines, tgt_lines)):
            ...
            if corpus == "train" and len(src.split()) < args.min_train_doclen:
                logger.warning(
                    "Skip too short document: corpus=train, doc=%s, sents=%s, tokens=%s"
            ...
            if any(l > args.max_tokens for l in srcs_len):
                logger.warning(
                    "Source doc has too long segment: corpus=%s, doc=%s, sents=%s, seg_len=%s, max_len=%s."
            ...
            if any(l > args.max_tokens for l in tgts_len):
                logger.warning(
                    "Target doc has too long segment: corpus=%s, doc=%s, sents=%s, seg_len=%s, max_len=%s."
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        level=logging.INFO, filename="./data_builder.log", format="[%(asctime)s %(levelname)s] %(message)s"
    )
