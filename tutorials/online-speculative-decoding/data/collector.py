...
class Collector:
    def __init__(self, name, *args, **kwargs) -> None:
        if "cache_dir" not in kwargs:
            ...
            kwargs["cache_dir"] = Path(Path.home(), "Data", ".cache", "datasets").as_posix()

        self.dataset = load_dataset(name, *args, **kwargs)
        self.name = name.replace("/", "_")

    def get_output_filename(self, split, prefix, raw=False):
        if prefix is not None:
            name = f"raw_data/{prefix}_{self.name}_{split}"
        else:
            name = f"raw_data/{self.name}_{split}"
    ...
    def collect(self, splits, transform, size=None, prefix=None, split_train=False):
        ...
        cases = []
        for split in splits:
            rawfile = self.get_output_filename(split, prefix, raw=True)
