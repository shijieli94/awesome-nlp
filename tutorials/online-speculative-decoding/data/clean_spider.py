...
def transform(i, case, need_label=False):
    ...
    if need_label:
        case["conversation"] = [
            {"role": "user", "content": SQL_prompt + case["question"]},
            {"role": "assistant", "content": " ".join(case["query_toks_no_value"])},
        ]
    else:
        case["conversation"] = [{"role": "user", "content": SQL_prompt + case["question"]}]
    return case
