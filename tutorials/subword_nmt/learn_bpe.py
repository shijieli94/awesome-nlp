    ...
    for j, word, old_word, freq in changed:
        ...
        while True:
            ...
            if i < len(old_word) - 1 and old_word[i + 1] == second:
                if i:
                    # if i > 0, there must be a symbol before it,
                    # assuming a symbol sequence "A B C", if "B C" is merged, reduce the frequency of "A B"
                    prev = old_word[i - 1 : i + 1]
                    stats[prev] -= freq
                    indices[prev][j] -= 1
                if i < len(old_word) - 2:
                    # in this case, there must be a symbol after it,
...
def prune_stats(stats, big_stats, threshold):
    ...
    for item, freq in list(stats.items()):
        if freq < threshold:
            del stats[item]
            if freq < 0:
                # if freq < 0, this means item has been deleted in the last prune,
                # so we subtract it from the big_stats
                big_stats[item] += freq
            else:
                big_stats[item] = freq
...
def learn_bpe(
    infile, outfile, num_symbols, min_frequency=2, verbose=False, is_dict=False, total_symbols=False, num_workers=1
):
...
if __name__ == "__main__":
    ...
    if os.path.isdir(newdir):
        warnings.warn(
            "this script's location has moved to {0}. "
            "This symbolic link will be removed in a future version. "
