...
def j_make_causal_mask_multilevel(
    ...
    level_offset = (
        tgt_len - (sum(level_sizes) + 1) - (len(guess) if guess is not None else 0)
    )  # offset when you guess multiple tokens and not copy kv-cache
    ...
    if guess is not None:
        ...
        if guess_size == 2:
            ...
            small_m3 = torch.tensor(
                [0, torch.finfo(dtype).min, torch.finfo(dtype).min, torch.finfo(dtype).min]
            ).repeat(lguess // 4)[:-3]
            mask[-lguess:, -lguess:] = (
                mask[-lguess:, -lguess:]
                .fill_diagonal_(0)
            ...
            mask[-lguess:, -lguess:] = (
                ...
                .diagonal_scatter(small_m2, -2)
                .diagonal_scatter(small_m3, -3)
                .diagonal_scatter(small_m4, -4)
            )
        elif guess_size == 6:
            small_m1 = torch.tensor([0, 0, 0, 0, 0, torch.finfo(dtype).min]).repeat(lguess // 6)[:-1]
            small_m2 = torch.tensor([0, 0, 0, 0, torch.finfo(dtype).min, torch.finfo(dtype).min]).repeat(lguess // 6)[
                :-2
            ]
            small_m3 = torch.tensor(
            ...
            small_m4 = torch.tensor(
                [0, 0, torch.finfo(dtype).min, torch.finfo(dtype).min, torch.finfo(dtype).min, torch.finfo(dtype).min]
            ).repeat(lguess // 6)[:-4]
            ...
            small_m3 = torch.tensor(
                [0, 0, 0, 0, torch.finfo(dtype).min, torch.finfo(dtype).min, torch.finfo(dtype).min]
            ).repeat(lguess // 7)[:-3]
            small_m4 = torch.tensor(
                [
                    0,
    ...
    for ll in range(len(level_sizes)):
        ...
        mask[
            all_offset + tiny_mask_size * ll : all_offset + tiny_mask_size * (ll + 1),
            all_offset : all_offset + tiny_mask_size,
        ].masked_fill_(hm, 0)
        for row in range(1, ll + 1):
            mask[
                all_offset + tiny_mask_size * ll : all_offset + tiny_mask_size * (ll + 1),
                all_offset + tiny_mask_size * row : all_offset + tiny_mask_size * (row + 1),
            ].fill_diagonal_(0)

    if past_key_values_length > 0 or la_mask_offset > 0:
        mask = torch.cat(
            [torch.zeros(tgt_len, past_key_values_length + la_mask_offset, dtype=dtype, device=device), mask], dim=-1
        )
...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def _flash_attention_forward(
        ...
        assert (seqlen_q == query_states.size(1) and seqlen_k == key_states.size(1)) or sum(lookahead[:3]) + sum(
            lookahead[4:]
        ) == 0, f"Setups: {lookahead}, {seqlen_q}, {seqlen_k}, {query_states.size(1)}, {key_states.size(1)}"
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def j_prepare_decoder_attention_mask(
    ...
    def LlamaModeljforward(
        ...
        level_offset = (
            seq_length - (sum(level_sizes) + 1) - (len(guess) if guess is not None else 0)
        )  # offset when you guess multiple tokens and not copy kv-cache
        dist_offset = 1 + level_sizes[0] - level_sizes[-1]  # offset for distributed inference
        ...
        lookahead = [
            level_sizes[-1],
            len(level_sizes) + 1,
            len(guess) // guess_size if guess is not None else 0,
            0,
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def jforward_multilevel(
        ...
        assert bool(continue_all) is False
        ...
        if guess_tokens is not None:
            ...
            input_ids = torch.cat((input_ids, input_ids.new_tensor(all_past + guess_tokens).unsqueeze(0)), dim=1)
            position_ids = torch.cat((position_ids, input_ids.new_tensor(ids_list + guess_ids).unsqueeze(0)), dim=1)
            attention_mask = torch.cat((attention_mask, input_ids.new_ones(1, attn_size + len(guess_tokens))), dim=1)

        else:
            input_ids = torch.cat((input_ids, input_ids.new_tensor(all_past).unsqueeze(0)), dim=1)
            position_ids = torch.cat((position_ids, input_ids.new_tensor(ids_list).unsqueeze(0)), dim=1)
            attention_mask = torch.cat((attention_mask, input_ids.new_ones(1, attn_size)), dim=1)
