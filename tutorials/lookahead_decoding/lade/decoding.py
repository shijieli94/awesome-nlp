...
COLOR_PRINT = bool(os.environ.get("COLOR_PRINT", 0))
CHAT = bool(os.environ.get("CHAT", 0))
USE_LADE = bool(os.environ.get("USE_LADE", 0))


def greedy_search_proxy(self, *args, **kwargs):
    if USE_LADE:
        return jacobi_greedy_search_multilevel(self, chat=CHAT, *args, **kwargs)
...
def sample_proxy(self, *args, **kwargs):
    if USE_LADE:
        return jacobi_sample_multilevel(self, chat=CHAT, *args, **kwargs)
...
def jacobi_greedy_search_multilevel(
    ...
    **model_kwargs,
    ...
    while True:
        ...
        assert bool(return_dict_in_generate) is False
            ...
            for idx, kv in enumerate(outputs.past_key_values):
                ...
                if max_hit > 0:
                    kv[0][:, :, outputs.kvcache_len : outputs.kvcache_len + max_hit, :] = kv[0][
                        :, :, offset_kv_cache : offset_kv_cache + max_hit, :
                    ]
        ...
        if chat and LOCAL_RANK == 0:
            ...
            if COLOR_PRINT:
                ...
                if max_hit > 1:
                    not_hit = self.tokenizer.decode(
                        all_old_tokens[: -max_hit + 1],
                        skip_special_tokens=True,
                        spaces_between_special_tokens=False,
                        clean_up_tokenization_spaces=True,
                    )
        ...
        input_ids = torch.cat([input_ids, next_tokens.new_tensor(hits[: max_hit + 1]).unsqueeze(0)], dim=-1)
    ...
    if DEBUG and LOCAL_RANK == 0:
        ...
        print(
            "Generated tokens: ",
            len(all_old_tokens) - init_len,
            "Total steps: ",
            steps,
            " Compression ratio: ",
            round((len(all_old_tokens) - init_len) / steps, 2),
        )
        print("======================================================================================", end="")
        CONFIG_MAP["log"].append(
            [len(all_old_tokens) - init_len, steps, round((len(all_old_tokens) - init_len) / steps, 2)]
        )
...
def jacobi_sample_multilevel(
    ...
    **model_kwargs,
    ...
    for warper in logits_warper:
        ...
        assert (
            type(warper) == TemperatureLogitsWarper
            or type(warper) == TopKLogitsWarper
            or type(warper) == TopPLogitsWarper
        ), f"please set top_k=0.0 and top_p=1.0 {warper}"
    ...
    while True:
        ...
        if past_tokens[1] is None:
            ...
            past_tokens[fill_level + 1] = torch.argmax(outputs.inp_logits, dim=-1)[0].tolist()[
                1:
            ]  # fill window with argmax
            ...
            if guess_tokens is not None:
                ...
                for idx_in_ngram in range(GUESS_SIZE):
                    ...
                    while g_idx < len(guess_indices):
                        ...
                        prob_accept = min(
                            1, probs_next[draft_guess].item()
                        )  # min(1, prob_llm/prob_draft) #use argmax, prob_draft is 1
        ...
        for idx, kv in enumerate(outputs.past_key_values):
            for hh in range(max_hit):
                ...
                kv[0][:, :, outputs.kvcache_len + hh, :] = kv[0][
                    :, :, outputs.step_len - len(guess_tokens) + max_hit_idx * GUESS_SIZE + hh, :
                ]
                kv[1][:, :, outputs.kvcache_len + hh, :] = kv[1][
                    :, :, outputs.step_len - len(guess_tokens) + max_hit_idx * GUESS_SIZE + hh, :
                ]
            past_key_values.append(
                (kv[0][:, :, : outputs.kvcache_len + max_hit, :], kv[1][:, :, : outputs.kvcache_len + max_hit, :])
        ...
        input_ids = torch.cat(
            [
                input_ids,
                torch.tensor(hits[: max_hit + 1], device=input_ids.device, dtype=input_ids.dtype).unsqueeze(0),
            ],
            dim=-1,
        )
