...
class VQ_VAE(nn.Module):
    ...
    def encode(self, x):
        h1 = self.relu(self.fc1(x))
        h2 = self.fc2(h1)
        return h2.view(-1, self.emb_size, self.hidden // self.emb_size)
    ...
    def forward(self, x):
        ...
        emb = self.emb(z_e.detach())[0].view(-1, self.emb_size, self.hidden // self.emb_size)
        return self.decode(z_q), z_e, emb

    def sample(self, size):
        sample = torch.randn(size, self.emb_size, self.hidden // self.emb_size)
        if self.cuda():
            sample = sample.cuda()
