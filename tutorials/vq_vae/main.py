...
models = {
    "custom": {"vqvae": VQ_CVAE, "vqvae2": VQ_CVAE2},
    "imagenet": {"vqvae": VQ_CVAE, "vqvae2": VQ_CVAE2},
    "cifar10": {"vae": CVAE, "vqvae": VQ_CVAE, "vqvae2": VQ_CVAE2},
...
def main(args):
    ...
    model_parser.add_argument(
        "--model", default="vae", choices=["vae", "vqvae"], help="autoencoder variant to use: vae | vqvae"
    )
    model_parser.add_argument(
    ...
    model_parser.add_argument(
        "-k", "--dict-size", type=int, dest="k", metavar="K", help="number of atoms in dictionary"
    )
    ...
    training_parser.add_argument(
        "--dataset_dir_name", default="", help="name of the dir containing the dataset if dataset == custom"
    )
    training_parser.add_argument("--data-dir", default="./datasets", help="directory containing the dataset")
    training_parser.add_argument(
    ...
    training_parser.add_argument("--gpus", default="0", type=str, help="gpus used for training - e.g 0,1,3")
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets_classes[args.dataset](
            dataset_train_dir, transform=dataset_transforms[args.dataset], **dataset_train_args[args.dataset]
        ),
    ...
    test_loader = torch.utils.data.DataLoader(
        datasets_classes[args.dataset](
            dataset_test_dir, transform=dataset_transforms[args.dataset], **dataset_test_args[args.dataset]
        ),
        batch_size=args.batch_size,
        shuffle=False,
        **kwargs,
    ...
    for epoch in range(1, args.epochs + 1):
        train_losses = train(
            epoch, model, train_loader, optimizer, args.cuda, args.log_interval, save_path, args, writer
        )
        ...
        if (batch_idx + 1) % log_interval == 0:
            ...
            logging.info(
                ...
                " {time:3.2f}   {loss}".format(
                    ...
                    percent=int(100.0 * (batch_idx + 1) / len(train_loader)),
...
def save_reconstructed_images(data, epoch, outputs, save_path, name):
    ...
    comparison = torch.cat([data[:n], outputs.view(batch_size, size[1], size[2], size[3])[:n]])
    save_image(comparison.cpu(), os.path.join(save_path, name + "_" + str(epoch) + ".png"), nrow=n, normalize=True)
