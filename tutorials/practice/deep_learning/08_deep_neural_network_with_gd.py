...
def L_layer_model(X, Y, layer_dims, learning_rate, num_iterations, gradient_descent="bgd", mini_batch_size=64):
    ...
    if gradient_descent == "bgd":
        ...
        shuffled_X = X[:, permutation]
        shuffled_Y = Y[:, permutation].reshape((1, m))
        num_iterations = (num_iterations - 1) // m + 1
        for i in range(0, num_iterations):
            for j in range(0, m):  # 每次训练一个样本
                ...
                parameters = update_parameters(parameters, grads, learning_rate)
                if _updates % 1000 == 0:
                    print("example size {}: {}".format(_updates, cost))
                    costs.append(cost)
                _updates += 1
    elif gradient_descent == "mini-batch":
        seed = 0
        update_per_epoch = (m - 1) // mini_batch_size + 1
        num_iterations = (num_iterations - 1) // update_per_epoch + 1
        for i in range(0, num_iterations):
            ...
            for minibatch in minibatches:
                ...
                parameters = update_parameters(parameters, grads, learning_rate)
                if _updates % 1000 == 0:
                    print("Cost after iteration {}: {}".format(_updates, cost))
                    costs.append(cost)
                _updates += 1
...
def DNN(
    ...
    parameters = L_layer_model(
        X_train, y_train, layer_dims, learning_rate, num_iterations, gradient_descent, mini_batch_size
    )
...
if __name__ == "__main__":
    ...
    # print(accuracy)
    # use sgd
    # accuracy = DNN(X_train, y_train, X_test, y_test, [X_train.shape[0], 10, 5, 1], gradient_descent="sgd")
    # print(accuracy)
    # mini-batch
    accuracy = DNN(X_train, y_train, X_test, y_test, [X_train.shape[0], 10, 5, 1], gradient_descent="mini-batch")
    print(accuracy)
