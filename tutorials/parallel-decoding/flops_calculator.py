...
class TransformerHparams(object):
    ...
    def get_embedding_flops(self, output=False):
        ...
        if self.e != self.h or output:
            embedding_flops.update(dict(hidden_kernel=2 * self.h * self.e, hidden_bias=self.e if output else self.h))
    ...
    def get_binary_classification_flops(self):
        classification_flops = dict(
            hidden=2 * self.h * self.h, hidden_bias=self.h, hidden_act=ACTIVATION_FLOPS * self.h, logits=2 * self.h
        )
    ...
    def get_train_flops(self, batch_size, train_steps, discriminator=False, use_backprop=True):
        ...
        return (
            ...
            * (
                ...
                + (self.get_binary_classification_flops() if discriminator else self.get_embedding_flops(output=True))
            )
        )
...
def get_electra_train_flops(
    h_d, l_d, h_g, l_g, batch_size, train_steps, tied_embeddings, e=None, s=512, output_frac=0.15625
):
    """Get the FLOPs needed for  pre-training ELECTRA."""
    if e is None:
        e = h_d
    disc = TransformerHparams(h_d, l_d, s=s, e=e, output_frac=output_frac).get_train_flops(
        batch_size, train_steps, True
    )
    gen = TransformerHparams(h_g, l_g, s=s, e=e if tied_embeddings else None, output_frac=output_frac).get_train_flops(
...
MODEL_FLOPS = collections.OrderedDict(
    [
        ...
        ("gpt", TransformerHparams(768, 12, v=40000, output_frac=1.0).get_train_flops(128, 960800)),
        ...
        ("albert", TransformerHparams(4096, 12, v=30000, e=128).get_train_flops(4096, 1.5e6)),
        (
            "t5_11b",
            TransformerHparams(
                1024,  # hidden size
        ...
        (
            "Shallow + KD",
            TransformerHparams(512, 13, i=2048).get_train_flops(250, 300000)
            + TransformerHparams(512, 13, i=2048, heads=16).get_train_flops(250, 300000, use_backprop=False),
        ),
        (
            "DSLP",
            TransformerHparams(512, 12, i=2048).get_train_flops(250, 300000)
            + TransformerHparams(512, 12, i=2048, heads=16).get_train_flops(250, 300000, use_backprop=False),
        ),
        (
            "F-VAE",
            TransformerHparams(512, 12, i=2048).get_train_flops(250, 300000)
        ...
        (
            "DisCo",
            TransformerHparams(512, 12, i=2048).get_train_flops(250, 300000)
            + TransformerHparams(1024, 24, i=4096, heads=16).get_train_flops(250, 300000, use_backprop=False),
        ),
