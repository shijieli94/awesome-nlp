...
from .utils.bench_scorer import Scorer
from .utils.utils import check_zero_division, retrieve_model_name
...
class MTBenchmarker(object):
        ...
        for x in pbar:
            try:
                ...
                for decoder, name in zip(self.decoders, scorers):
                    ...
                    trans = self.dataset.tokenizer.decode(trans[0], skip_special_tokens=True)
    ...
    def _compute_time_table(self, scorers, grid):
        if len(scorers) == 1:
            ...
            time_speedup = [
                check_zero_division(comp_scorer.tot_mean_time, scorer.tot_mean_time) for scorer in scorers.values()
            ]
            iter_speedup = [
                check_zero_division(comp_scorer.tot_mean_iter, scorer.tot_mean_iter) for scorer in scorers.values()
            ]
    ...
    def _compute_bleu_table(self, scorers, grid):
        ...
        bleu_table = tabulate(
            ...
            headers=["Metrics"] + [name for name in scorers],
            tablefmt=grid,
        )
    ...
    def _compute_best_algorithm(scorers):
        return scorers.get(min(scorers, key=lambda x: scorers[x].current_time)).acronym
    ...
    def _compute_postfix(self, scorers, best_algorithms, curr_alg):
        ...
        if len(scorers) == 1:
            ...
            postfix = {
                scorer.acronym: (round(scorer.tot_mean_time, 3), round(scorer.tot_mean_iter, 3)),
                "ca": curr_alg,
                "ba": best_alg,
            }
