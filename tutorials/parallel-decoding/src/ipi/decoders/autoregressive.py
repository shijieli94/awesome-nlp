...
class AutoregressiveDecoder(MTDecoder):
    ...
    def initialize(self):
        return self.initializer.init_translation()[0] if self.initializer is not None else None
    ...
    def compute_decode_kwargs(self, input_ids, *args, **kwargs):
        ...
        return {"init_tensor": init_tensor.clone(), "logits_preprocessor": logits_preprocessor}
    ...
    def decode(self, input_ids, attention_mask, init_tensor=None, logits_preprocessor=None, *args, **kwargs):
        ...
        if init_tensor is None:
            init_tensor = torch.tensor([self.pad_token_id], device=self.device).unsqueeze(0)
            ...
            index += 1
            total_res = torch.tensor([[init_tensor[:, 0], init_tensor[:, 1]]], device=self.device)
