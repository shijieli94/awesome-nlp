...
from ...viz.dependecy_graph import DecodingDependencyGraph
...
class JacobiDecoder(MTDecoder):
    ...
    def initialize(self, init_transl):
        return self.initializer.init_translation(init_transl.shape[-1])[0] if self.initializer is not None else None
    ...
    def decode(
        ...
        **kwargs
        ...
        if compute_ddg:
            ...
            ddg = DecodingDependencyGraph(model=self.model, tokenizer=self.tokenizer, gold_target=gold_target)
        ...
        for index in range(str_index, max_length):
            ...
            if compute_ddg:
                ddg.insert_one_element(total_res, gold_target, output_probs=output_probs)

            if stop_condition:
                break

        return (return_tensor, index, ddg) if compute_ddg else (return_tensor, index)
