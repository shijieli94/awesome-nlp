...
class BleuEvaluator(object):
    def __init__(self):
        self.metric = evaluate.load("sacrebleu", cache_dir=EVALUATE_DIR)
...
class BleuCalculator:
    ...
    def read_csv(path_csv):
        ...
        return {k: v for k, v in zip(csv_reader["#sentence"].tolist(), csv_reader["times"].tolist())}
    ...
    def compute_bleu_score(self):
        ...
        if "trans_beam" in file2data:
            beam_search = self._match_indices(file2data["trans_gs_jacobi"], file2data["trans_beam"])
            file2data["trans_beam"] = beam_search
        gold = self._match_indices(file2data["trans_gs_jacobi"], gold)

        file2score = [self._compute_bleu_score(file, file2data[file], gold) for file in file2data]
