...
class DecodingDependencyGraph(object):
    ...
    def insert_one_element(self, current_tensor, gold_tensor, index=None, output_probs=None):
        self.init_matrix.append((current_tensor[:, self.starting_index :] == gold_tensor[:, self.starting_index :]))
    ...
    def pretty_print(self, sample_index: int, sentence_id: str, method="basic", x_remap="text"):
        ...
        fig.update_xaxes(
            ...
            ticktext=[self.tokenizer.convert_ids_to_tokens([x])[0] if x_remap else str(x) for x in labels[1:]],
            tickangle=45,
        )
