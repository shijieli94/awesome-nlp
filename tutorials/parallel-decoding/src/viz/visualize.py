...
from ..ipi.decoders.jacobi import JacobiDecoder
from ..ipi.decoders.mt_decoding import MTDecoder, generate_target
from ..ipi.initializer import Initializer


def add_iteration_matrix(sample: dict, src_lang: str, tgt_lang: str, model, tokenizer, initializer, decoder):
...
def enrich_dataset(
    ...
    if run_dir.exists() and not force_recompute:
        ...
        decoder = JacobiDecoder(
            tokenizer=tokenizer, model=model, initializer=initializer, use_cache=False, device=device
        )
        ...
        json.dump(
            run_info, fp=(run_dir / "run_info.json").open("w", encoding="utf-8"), indent=4, default=lambda x: str(x)
        )
...
if __name__ == "__main__":
    ...
    if "en" in langs:
        dataset_key = f"{src_lang}-{tgt_lang}" if "en" == tgt_lang else f"{tgt_lang}-{src_lang}"
    ...
    run_info_hash: str = hashlib.md5(json.dumps(run_info).encode(encoding="utf-8")).hexdigest()
    ...
    for sample in dataset:
        ...
        print(f"Gold output: {tokenizer.decode(sample['gold_output'], skip_special_tokens=True)}")
        print()
        # input()
        # continue
        ...
        (images_dir / f"{sample['index']}.json").write_text(json.dumps(x, indent=4, sort_keys=True), encoding="utf-8")
