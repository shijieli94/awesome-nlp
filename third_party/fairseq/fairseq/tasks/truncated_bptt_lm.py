...
class TruncatedBPTTLMTask(FairseqTask):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        dataset = data_utils.load_indexed_dataset(split_path, self.dictionary, combine=combine)
        if dataset is None:
    ...
    def get_batch_iterator(
        self, dataset, num_workers=0, epoch=1, data_buffer_size=0, skip_remainder_batch=False, **kwargs
    ):
        can_reuse_epoch_itr = True
    ...
    def eval_lm_dataloader(
        ...
        if context_window > 0:
            raise NotImplementedError(
                "Transformer-XL doesn't need --context-window, try " "--model-overrides '{\"mem_len\":42}' instead "
            )
...
class TruncatedBPTTDataset(FairseqDataset):
    def __init__(
        self,
        dataset: List[torch.Tensor],  # ordered list of items
        sizes,
        src_vocab,
        tgt_vocab,
        ...
        *indices* might look like:
        ...
        and shard 1 will see items:
            ...
            [(0, [data[4], data[6]]),
             (1, [data[5], data[7]])]
        """
        indices = batchify(torch.arange(len(dataset)), global_batch_size)
    ...
    def __getitem__(self, i) -> Tuple[int, List[torch.Tensor]]:
        return (i, [self.dataset[idx] for idx in self.my_indices[:, i]])

    def collater(self, items: List[List[torch.Tensor]]):
        ...
        id, item = items[0]
        item = data_utils.collate_tokens(item, pad_idx=self.src_vocab.pad())
        B, T = item.size()

        # shift item one position over and append a padding token for the target
        target = torch.nn.functional.pad(item[:, 1:], (0, 1, 0, 0), value=self.tgt_vocab.pad())
