...
class SpeechDLM(FairseqLanguageModel):
    ...
    def build_model(cls, args, task):
        ...
        assert all(
            task.source_dictionaries[channel] == task.source_dictionary for channel in task.channels
        ), "Source dictionaries of all channels are expected to be the same!!!"
        assert all(
            task.target_dictionaries[channel] == task.target_dictionary for channel in task.channels
