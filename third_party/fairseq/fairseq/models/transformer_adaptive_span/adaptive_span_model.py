...
class SeqAttention(nn.Module):
    ...
    def __init__(self, hidden_size, nb_heads, attn_span, dropout, adapt_span_params, pers_mem_params, **kargs):
        ...
        if self.adapt_span_enabled:
            self.adaptive_span = AdaptiveSpan(attn_span=attn_span, nb_heads=nb_heads, **adapt_span_params, **kargs)

        self.persistent_memory = None
        if pers_mem_params["pers_mem_size"] > 0:
            self.persistent_memory = PersistentMemory(pers_mem_params["pers_mem_size"], nb_heads, hidden_size, dropout)
            if self.adapt_span_enabled:
...
class TransformerSeq(nn.Module):
    def __init__(self, vocab_size, d_model, n_heads, n_layers, attn_span, emb_dropout, adapt_io_params={}, **kargs):
        super().__init__()
        # token embeddings
        self.adapt_io = adapt_io_params["adapt_io_enabled"]
        if self.adapt_io:
            self.in_emb, self.out_emb = build_adaptive_io(vocab_size, d_model, **adapt_io_params)
        else:
        ...
        # position embeddings
        self.key_pe = nn.Parameter(torch.randn(1, d_model // n_heads, attn_span))

        self.layers = nn.ModuleList()
        self.layers.extend(
            TransformerSeqLayer(hidden_size=d_model, nb_heads=n_heads, attn_span=attn_span, **kargs)
