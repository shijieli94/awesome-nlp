...
class SampledMultiEpochDataset(SampledMultiDataset):
    ...
    def _next_global_indices(self, epoch):
        ...
        if self.load_next_shard is None:
            ...
            logger.info(
                "to load next epoch/shard in next load_dataset: " f"epoch={epoch}/shard_epoch={self.shard_epoch}"
            )
    ...
    def _next_virtual_epoch(self, epoch):
        ...
        if index == 0 or self._random_global_indices is None:
            ...
            logger.info(
                "establishing a new set of global virtual indices for " f"epoch={epoch}/shard_epoch={self.shard_epoch}"
            )
